...
export default function WorkspaceScreen({ email, groups, projects, busy, error, savedNotice, onRefresh, onStartSession, onOpenProject, onDisconnect }: {
  ...
  const candidates = useMemo(() => rankDiscoveryCandidates(groups, projects.map((project) => project.businessContext.observed.title)), [groups, projects])
  ...
  return <main className="dashboard-main">
      ...
      <section className="active-projects"><header><h2>検証中の仮説</h2><p>未確認の項目と、次に進める検証です。</p></header>
        ...
        {adoptedProjects.length > 0 && <div className="reduction-summary"><h3><TrendingDown size={18} />採用した仮説の効果</h3>
          {adoptedProjects.map((project) => {
            const reduction = computeReduction(project.businessContext.observed, groups)
            return <button key={project.id} type="button" className="reduction-row" onClick={() => onOpenProject(project)}>
              <strong>{project.taskName}</strong>
    ...
    <details className="saved-projects-list"><summary>保存した仮説をすべて見る（{projects.length}件）</summary>
      {projects.length ? <div className="project-list">{projects.map((project) => <button key={project.id} type="button" onClick={() => onOpenProject(project)}><span><strong>{project.taskName}</strong><small>{new Intl.DateTimeFormat('ja-JP', { dateStyle: 'medium' }).format(new Date(project.updatedAt))}</small></span><span className={`project-status status-${project.status.toLowerCase()}`}>{projectStatusLabels[project.status]}</span><ArrowRight size={20} /></button>)}</div> : <p className="empty-projects-inline">保存した仮説はまだありません。</p>}
    </details>
