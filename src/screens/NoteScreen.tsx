...
export default function NoteScreen({ project, currentGroups, onAddContext, onUpdateHypothesis, onStatus, onDelete }: {
  ...
  return <main className="project-detail-main">
    ...
    {contextDirty && <section className="hypothesis-update-banner"><div><strong>業務モデルが更新されました</strong><p>保存中の仮説はまだ更新前の内容です。画面下の「仮説を更新する」で反映できます。</p></div></section>}
    ...
    <div className="action-bar">
      <label className="action-bar-field">この仮説の扱い
      ...
      <div className="action-bar-hint">{status === 'error' && <span className="calendar-error" role="alert">{errorMessage}</span>}</div>
      {contextDirty && <button type="button" className="primary-button" disabled={status === 'loading'} onClick={() => void run(onUpdateHypothesis)}>{status === 'loading' ? '更新中' : '仮説を更新する'}</button>}
    </div>
    {activeUnknownId && (() => {
      const unknown = unknowns.find((item) => item.id === activeUnknownId)
      if (!unknown) return null
      const question = { ...questionForContext(unknown.dimension).questions[0], id: unknown.id }
      return <QuestionDialog title={unknown.question} description={unknown.reason} busy={status === 'loading'} onClose={() => setActiveUnknownId(null)}>
        {errorMessage && <p className="calendar-error" role="alert">{errorMessage}</p>}
